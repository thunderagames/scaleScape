...
describe('settings store', () => {
  ...
  it('given_pending_language_and_visibility_changes_when_saving_settings_then_publishes_all_values_together', () => {
    ...
    store.subscribe((settings) => observed_settings.push(settings))
     ...
     store.setSettings({ language: 'es', note_naming: 'letter', show_piano: false, show_guitar: true, show_bass: true, show_ukulele: true, show_scale_description: true, ear_gym_streak: 0, volume: 0.7, tempo_bpm: 120, metronome_bpm: 120, guitar_tuning_semitones: 0, bass_tuning_semitones: 0, ukulele_tuning_semitones: 0, last_root: 4, last_formula: 'dorian', guided_start_completed: false })

      expect(observed_settings).toEqual([{ language: 'es', note_naming: 'letter', show_piano: false, show_guitar: true, show_bass: true, show_ukulele: true, show_scale_description: true, ear_gym_streak: 0, volume: 0.7, tempo_bpm: 120, metronome_bpm: 120, guitar_tuning_semitones: 0, bass_tuning_semitones: 0, ukulele_tuning_semitones: 0, last_root: 4, last_formula: 'dorian', guided_start_completed: false }])
  })
  ...
  it('given_saved_settings_when_creating_a_new_store_then_loads_them_from_local_storage', () => {
    ...
    const first_store = createSettingsStore()
     first_store.setSettings({ language: 'es', note_naming: 'letter', show_piano: false, show_guitar: true, show_bass: false, show_ukulele: false, show_scale_description: false, ear_gym_streak: 3, volume: 0.45, tempo_bpm: 150, metronome_bpm: 90, guitar_tuning_semitones: -2, bass_tuning_semitones: 3, ukulele_tuning_semitones: 2, last_root: 9, last_formula: 'lydian', guided_start_completed: true })

    const second_store = createSettingsStore()

      expect(second_store.getSettings()).toEqual({ language: 'es', note_naming: 'letter', show_piano: false, show_guitar: true, show_bass: false, show_ukulele: false, show_scale_description: false, ear_gym_streak: 3, volume: 0.45, tempo_bpm: 150, metronome_bpm: 90, guitar_tuning_semitones: -2, bass_tuning_semitones: 3, ukulele_tuning_semitones: 2, last_root: 9, last_formula: 'lydian', guided_start_completed: true })
  })
  ...
  it('given_invalid_saved_settings_when_creating_a_store_then_uses_safe_defaults', () => {
    ...
    const store = createSettingsStore()
      ...
      expect(store.getSettings()).toEqual({ language: 'en', note_naming: 'letter', show_piano: true, show_guitar: true, show_bass: true, show_ukulele: true, show_scale_description: true, ear_gym_streak: 0, volume: 0.7, tempo_bpm: 120, metronome_bpm: 120, guitar_tuning_semitones: 0, bass_tuning_semitones: 0, ukulele_tuning_semitones: 0, last_root: 4, last_formula: 'dorian', guided_start_completed: false })
  })
  ...
  it('given_legacy_saved_settings_when_creating_a_store_then_defaults_missing_streak', () => {
    ...
    const store = createSettingsStore()
      ...
      expect(store.getSettings()).toEqual({ language: 'es', note_naming: 'letter', show_piano: true, show_guitar: false, show_bass: true, show_ukulele: true, show_scale_description: true, ear_gym_streak: 0, volume: 0.7, tempo_bpm: 120, metronome_bpm: 120, guitar_tuning_semitones: 0, bass_tuning_semitones: 0, ukulele_tuning_semitones: 0, last_root: 4, last_formula: 'dorian', guided_start_completed: false })
   })

  it('given_legacy_saved_settings_when_creating_a_store_then_defaults_metronome_bpm', () => {
    window.localStorage.setItem('scalescape.settings.v1', JSON.stringify({ language: 'en', show_piano: true, show_guitar: true }))

    expect(createSettingsStore().getSettings().metronome_bpm).toBe(120)
  })

  it('given_invalid_saved_metronome_bpm_when_creating_a_store_then_defaults_metronome_bpm', () => {
    window.localStorage.setItem('scalescape.settings.v1', JSON.stringify({ language: 'en', show_piano: true, show_guitar: true, metronome_bpm: 251 }))
  ...
  it('given_legacy_saved_settings_when_creating_a_store_then_defaults_bass_tuning_to_standard', () => {
   ...
   it('given_legacy_saved_settings_when_creating_a_store_then_defaults_ukulele_to_visible_and_standard', () => {
    window.localStorage.setItem('scalescape.settings.v1', JSON.stringify({ language: 'en', show_piano: true, show_guitar: true }))

    expect(createSettingsStore().getSettings().show_ukulele).toBe(true)
    expect(createSettingsStore().getSettings().ukulele_tuning_semitones).toBe(0)
  })
  ...
  it('given_legacy_saved_settings_when_creating_a_store_then_defaults_scale_description_to_visible', () => {
    window.localStorage.setItem('scalescape.settings.v1', JSON.stringify({ language: 'en', show_piano: true, show_guitar: true }))

    expect(createSettingsStore().getSettings().show_scale_description).toBe(true)
  })
