...
export interface SettingsStore {
  ...
  setInstrumentVisibility(instrument: 'piano' | 'guitar' | 'bass', is_visible: boolean): AppSettings
...
export function createSettingsStore(initial_language: Language = 'en', diagnostics: EventLoggerPort = { log: () => undefined }): SettingsStore {
  ...
  return {
    ...
    setInstrumentVisibility: (instrument, is_visible) => publish({ ...settings, [instrument === 'piano' ? 'show_piano' : instrument === 'guitar' ? 'show_guitar' : 'show_bass']: is_visible }),
    subscribe: (listener) => {
