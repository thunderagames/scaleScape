...
export function renderExploreScreen(container: HTMLElement, application: ExploreApplication, playback: PlaybackPort, settings: SettingsStore, diagnostics: EventLoggerPort = createDiagnosticsLogger(), guided_start?: ExploreGuidedStartPort): void {
     ...
     <main class="explore-shell">
      <header class="explore-header">
       ...
       <section id="instrument-region" class="instrument-grid">
               <article id="piano-card" class="instrument-card" aria-labelledby="piano-title"><div class="section-heading"><h2 id="piano-title"></h2><div class="instrument-heading-actions"><button id="piano-metronome" class="control-button control-button--icon metronome-trigger" type="button" aria-pressed="false"><svg class="metronome-icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false"><path class="metronome-body" d="M9 27 13.2 7h5.6L23 27H9Z"/><path class="metronome-base" d="M6 27h20M8 29h16"/><path class="metronome-scale" d="M13.5 11h5M13 14h6M12.5 17h7"/><path class="metronome-pendulum" d="M16 17.5 23 11"/><circle class="metronome-weight" cx="20.4" cy="13.6" r="1.7"/></svg></button><div class="help-cluster"><button id="piano-help-button" class="help-button" type="button" aria-expanded="false" aria-controls="piano-help">?</button><div id="piano-help" class="help-copy" role="tooltip"></div></div></div></div><p id="piano-metronome-hint" class="metronome-hint"></p><div id="piano-view" class="piano-view"></div></article>
               <article id="guitar-card" class="instrument-card" aria-labelledby="guitar-title"><div class="section-heading"><h2 id="guitar-title"></h2><div class="instrument-heading-actions"><button id="guitar-metronome" class="control-button control-button--icon metronome-trigger" type="button" aria-pressed="false"><svg class="metronome-icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false"><path class="metronome-body" d="M9 27 13.2 7h5.6L23 27H9Z"/><path class="metronome-base" d="M6 27h20M8 29h16"/><path class="metronome-scale" d="M13.5 11h5M13 14h6M12.5 17h7"/><path class="metronome-pendulum" d="M16 17.5 23 11"/><circle class="metronome-weight" cx="20.4" cy="13.6" r="1.7"/></svg></button><div class="help-cluster"><button id="guitar-help-button" class="help-button" type="button" aria-expanded="false" aria-controls="guitar-help">?</button><div id="guitar-help" class="help-copy" role="tooltip"></div></div></div></div><p id="guitar-metronome-hint" class="metronome-hint"></p><div id="guitar-view" class="guitar-view"></div></article>
                <article id="bass-card" class="instrument-card" aria-labelledby="bass-title"><div class="section-heading"><h2 id="bass-title"></h2><div class="instrument-heading-actions"><button id="bass-metronome" class="control-button control-button--icon metronome-trigger" type="button" aria-pressed="false"><svg class="metronome-icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false"><path class="metronome-body" d="M9 27 13.2 7h5.6L23 27H9Z"/><path class="metronome-base" d="M6 27h20M8 29h16"/><path class="metronome-scale" d="M13.5 11h5M13 14h6M12.5 17h7"/><path class="metronome-pendulum" d="M16 17.5 23 11"/><circle class="metronome-weight" cx="20.4" cy="13.6" r="1.7"/></svg></button></div></div><p id="bass-metronome-hint" class="metronome-hint"></p><div id="bass-view" class="guitar-view"></div></article><article id="ukulele-card" class="instrument-card" aria-labelledby="ukulele-title"><div class="section-heading"><h2 id="ukulele-title"></h2><div class="instrument-heading-actions"><button id="ukulele-metronome" class="control-button control-button--icon metronome-trigger" type="button" aria-pressed="false"><svg class="metronome-icon" viewBox="0 0 32 32" aria-hidden="true" focusable="false"><path class="metronome-body" d="M9 27 13.2 7h5.6L23 27H9Z"/><path class="metronome-base" d="M6 27h20M8 29h16"/><path class="metronome-scale" d="M13.5 11h5M13 14h6M12.5 17h7"/><path class="metronome-pendulum" d="M16 17.5 23 11"/><circle class="metronome-weight" cx="20.4" cy="13.6" r="1.7"/></svg></button></div></div><p id="ukulele-metronome-hint" class="metronome-hint"></p><div id="ukulele-view" class="guitar-view"></div></article>
       </section>
     </main>
  ...
  const guided_progress_text = container.querySelector<HTMLElement>('#guided-progress-text')
   ...
   const metronome_buttons = Array.from(container.querySelectorAll<HTMLButtonElement>('.metronome-trigger'))
   const metronome_hints = Array.from(container.querySelectorAll<HTMLElement>('.metronome-hint'))
        if (!root_select || !formula_select || !scale_selector || !play_scale || !play_chord || !scale_selector_modal || !close_scale_selector || !cancel_scale_selector || !apply_scale_selector || !scale_help_button || !scale_help_copy || !piano_help_button || !piano_help_copy || !guitar_help_button || !guitar_help_copy || !scale_caption || !scale_formula_info || !scale_notes || !note_detail || !scale_description || !piano_view || !guitar_view || !bass_view || !ukulele_view || !piano_card || !guitar_card || !bass_card || !ukulele_card || !guided_progress || !guided_progress_text || !guided_progress_action || metronome_buttons.length !== 4 || metronome_hints.length !== 4) throw new Error('Explore screen elements were not found')
     ...
     ui.scale_selector.setAttribute('aria-label', translation.scale_controls)
      ...
      ui.formula_select.value = current_state.formula_id
       ...
       ui.play_chord.textContent = translation.play_chord
        ...
        metronome_buttons.forEach((button, index) => {
          const is_active = playback.getPlaybackState().is_metronome_playing
          button.setAttribute('aria-label', is_active ? translation.stop_metronome : translation.start_metronome)
          button.title = is_active ? translation.stop_metronome : translation.start_metronome
          button.setAttribute('aria-pressed', String(is_active))
          button.classList.toggle('is-active', is_active)
          const hint = metronome_hints[index]
          if (hint) hint.textContent = translation.metronome_settings_hint
        })
  }
  ...
  application.subscribe((state) => render(state))
  ...
  ui.apply_scale_selector.addEventListener('click', () => { change_scale_from_controls(); close_scale_selector_dialog() })
     ui.play_scale.addEventListener('click', async () => { if (is_playing) { await playback.stopMelodicPlayback(); is_playing = false; render(application.getState()); return } selected_pitch_classes = new Set(); const result = await playback.playScale(application.getState().scale_instance, getVisiblePlaybackInstruments(settings)); is_playing = result.ok; render(application.getState()) }); ui.play_chord.addEventListener('click', async () => { if (is_playing) { await playback.stopMelodicPlayback(); is_playing = false; render(application.getState()); return } selected_pitch_classes = new Set(); const state = application.getState(); const chord_notes = state.scale_instance.notes.filter((note) => note.degree === 1 || note.degree === 3 || note.degree === 5); if (chord_notes.length < 2) return; const result = await playback.playChord(state.scale_instance, getVisiblePlaybackInstruments(settings)); is_playing = result.ok; render(application.getState()) }); metronome_buttons.forEach((button) => button.addEventListener('click', async () => { if (playback.getPlaybackState().is_metronome_playing) await playback.stopMetronome(); else await playback.startMetronome(settings.getSettings().metronome_bpm); apply_translations() })); playback.subscribePlaybackState(apply_translations); playback.subscribe({ on_note_started: (note_index) => { const note = application.getState().scale_instance.notes[note_index]; if (note) { selected_pitch_classes = new Set([note.pitch_class]); render(application.getState()) } }, on_chord_started: (note_indexes) => { const chord_pitch_classes = note_indexes.map((note_index) => application.getState().scale_instance.notes[note_index]?.pitch_class).filter((pitch_class): pitch_class is number => pitch_class !== undefined); if (chord_pitch_classes.length > 0) { selected_pitch_classes = new Set(chord_pitch_classes); render(application.getState()) } }, on_stopped: () => { selected_pitch_classes = new Set(); is_playing = false; render(application.getState()) } }); settings.subscribe(() => render(application.getState())); application.subscribe(render); render(application.getState())
}
