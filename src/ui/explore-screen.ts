...
export function renderExploreScreen(container: HTMLElement, application: ExploreApplication, playback: PlaybackPort, settings: SettingsStore, diagnostics: EventLoggerPort = createDiagnosticsLogger(), guided_start?: ExploreGuidedStartPort): void {
     ...
     <main class="explore-shell">
      <header class="explore-header">
        ...
        <p id="app-intro" class="intro"></p>
         ...
         <div class="scale-selection-row"><p id="scale-selection-hint" class="scale-selection-hint"></p><button id="scale-selector" class="scale-selector" type="button" aria-haspopup="dialog"></button><button id="play-scale" type="button" aria-label=""></button></div>
       ...
       <section id="instrument-region" class="instrument-grid">
         <article id="piano-card" class="instrument-card" aria-labelledby="piano-title"><div class="section-heading"><h2 id="piano-title"></h2></div><div id="piano-view" class="piano-view"></div></article>
         <article id="guitar-card" class="instrument-card" aria-labelledby="guitar-title"><div class="section-heading"><h2 id="guitar-title"></h2></div><div id="guitar-view" class="guitar-view"></div></article>
       </section>
     </main>
     <dialog id="scale-selector-modal" aria-labelledby="scale-selector-title"><form class="scale-selector-form"><div class="settings-heading"><h2 id="scale-selector-title"></h2><button id="close-scale-selector" class="modal-close" type="button"></button></div><label><span id="root-label"></span><select id="root-select"></select></label><label><span id="mode-label"></span><select id="formula-select"></select></label><div class="settings-actions"><button id="cancel-scale-selector" type="button"></button><button id="apply-scale-selector" type="button"></button></div></form></dialog>
  `
  ...
  const guided_progress_action = container.querySelector<HTMLButtonElement>('#guided-progress-action')
   if (!root_select || !formula_select || !scale_selector || !scale_selection_hint || !play_scale || !scale_selector_modal || !close_scale_selector || !cancel_scale_selector || !apply_scale_selector || !scale_caption || !scale_notes || !note_detail || !piano_view || !guitar_view || !piano_card || !guitar_card || !guided_progress || !guided_progress_text || !guided_progress_action) throw new Error('Explore screen elements were not found')

   const ui = { root_select, formula_select, scale_selector, scale_selection_hint, play_scale, scale_selector_modal, close_scale_selector, cancel_scale_selector, apply_scale_selector, scale_caption, scale_notes, note_detail, piano_view, guitar_view, piano_card, guitar_card, guided_progress, guided_progress_text, guided_progress_action }
  ...
  function apply_translations(): void {
    ...
    ui.scale_selector.setAttribute('aria-label', translation.scale_controls)
     ...
     ui.play_scale.innerHTML = is_playing ? '<svg class="playback-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 5h3v14H7zM14 5h3v14h-3z"/></svg>' : '<svg class="playback-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m8 5 11 7-11 7V5Z"/></svg>'
  ...
  ui.scale_selector.addEventListener('click', () => { if (typeof ui.scale_selector_modal.showModal === 'function') ui.scale_selector_modal.showModal(); else ui.scale_selector_modal.setAttribute('open', '') })
  ...
  ui.play_scale.addEventListener('click', async () => { if (is_playing) { await playback.stopAll(); is_playing = false; render(application.getState()); return } selected_pitch_class = null; const result = await playback.playScale(application.getState().scale_instance, getVisiblePlaybackInstruments(settings)); is_playing = result.ok; render(application.getState()) }); playback.subscribe({ on_note_started: (note_index) => { const note = application.getState().scale_instance.notes[note_index]; if (note) { selected_pitch_class = note.pitch_class; render(application.getState()) } }, on_stopped: () => { selected_pitch_class = null; is_playing = false; render(application.getState()) } }); settings.subscribe(() => render(application.getState())); application.subscribe(render); render(application.getState())
}
