...
export interface StringedInstrumentViewOptions {
  ...
  readonly use_progression_roles?: boolean
}
...
export function renderStringedInstrument(options: StringedInstrumentViewOptions): void {
  const { container, model, translation, instrument, selected_pitch_classes, aria_label, note_naming, on_position_selected, on_preview, note_accessible_label, use_progression_roles = false } = options
  ...
  for (let index = 0; index <= model.fret_count; index += 1) {
    const cell = document.createElement('th')
    cell.scope = 'col'
    cell.textContent = String(model.start_fret + index)
    head.append(cell)
  }
  ...
  model.strings.forEach((string_model, string_index) => {
    ...
    string_model.positions.forEach((position, position_index) => {
      ...
      button.type = 'button'
      const role = use_progression_roles ? position.progression_role : position.primary_role
      button.className = `guitar-position ${is_scale_note ? use_progression_roles ? `progression-${role}` : role : 'outside-scale'} ${is_selected ? 'selected' : ''}`
      ...
      if (is_scale_note) {
        ...
        button.addEventListener('keydown', (event) => {
          ...
          if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
            ...
            const next_button = fret_buttons[next_string_index]?.[position_index]
